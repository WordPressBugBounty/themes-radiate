		...
		<div id="message" class="notice notice-success radiate-notice">
			...
			<div class="radiate-message__content">
				<div class="radiate-message__image">
					<img class="radiate-screenshot" src="<?php echo esc_url( get_template_directory_uri() ); ?>/screenshot.jpg" alt="<?php esc_attr_e( 'Radiate', 'radiate' ); ?>" /><?php // phpcs:ignore WordPress.XSS.EscapeOutput.OutputNotEscaped, Squiz.PHP.EmbeddedPhp.SpacingBeforeClose ?>
				</div>
		...
		if ( ! current_user_can( 'manage_options' ) ) {
			wp_send_json_error(
				array(
					...
					'errorMessage' => __( 'You do not have permission to perform this action.', 'radiate' ),
				)
			);
			exit;
		}
